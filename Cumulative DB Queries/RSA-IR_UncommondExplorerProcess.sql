...
WHERE
	procfn.FileName LIKE 'explorer.exe'
	AND pfn.filename NOT IN ('userinit.exe', 'winlogon.exe', 'WerFault.exe', 'taskmgr.exe')
	AND NOT (la.LaunchArguments like ' ' OR la.LaunchArguments like '')
	AND la.LaunchArguments NOT LIKE '%/factory,{%'
	AND pa2.Path NOT LIKE '%\onedrive\%'
